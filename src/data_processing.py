...
def offset_start_end_positions(
    ...
    def add_positions_for_roberta_llmv3(examples):
        ...
        contexts = [
                para_texts_df[para_texts_df["doc_id"] == doc_id][
                    "paras"
                ].values[0][idx_para_matched]
                for doc_id, idx_para_matched in zip(examples["doc_id"], examples["idx_para_matched"])
            ]

        answers = {'text' : [answer_text.strip() for answer_text in examples["answer_text"]]}
        answers['start_positions'] =  examples["answer_start"]
        answers['end_positions'] = examples["answer_end"]
        ...
        examples['answer_start_token_idx'], examples['answer_end_token_idx'] = positions_from_tokenized_input(tokenized_by_roberta, roberta_tokenizer, answers)
    ...
    dataset = dataset.map(
        ...
        batched=True,
        remove_columns=["answer_start", "answer_end"],
        batch_size=batch_size,
        load_from_cache_file=False,
        # fn_kwargs={
        #     'para_texts_df': para_texts_df, 
        #     'roberta_tokenizer': roberta_tokenizer, 
        #     'llmv3_processor': llmv3_processor,
        # roberta_max_length, llmv3_max_length, roberta_stride, llmv3_stride
        # }
    )
...
def positions_from_tokenized_input(tokenized_examples, tokenizer, answers):
        ...
        if len(answers["start_positions"]) != 0:
            ...
            if (
                ...
                try:
                    ...
                    logger.info("Failed to properly offset position of sample_index:{sample_index}")
