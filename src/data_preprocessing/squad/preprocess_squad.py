...
def offset_bboxes(bboxes: List[List[int]], to_offset_box: List[int]):
    """
    ...
    bboxes = [
        [
            bbox[0] - to_offset_box[0],
            bbox[1] - to_offset_box[1],
            bbox[2] - to_offset_box[0],
            bbox[3] - to_offset_box[1],
        ]
        for bbox in bboxes
    ]
    return bboxes
...
def refactor_json_files(json_paths: List[str]):
    """
    Adds "[" and "]" to the start and end respectively of JSONs whose paths
    are given as a list.
...
def html_to_pdf(html_path: str, pdf_path: str):
    ...
    try:
        ...
        logger.error(f"Binary files of 'wkhtmltopdf' are missing, make sure you have installed them and pointing to "
                     f"'{WKHTMLTOPDF_PATH}'")
...
def generate_para_images_for_complete_para(
    ...
    if matched_lines_bboxes:
        ...
        matched_words = page.get_text("words", clip=para_box)
...
def collect_docs(
    ...
    (dataset object), saves htmls of the all the wikipages, and finally
    saves the data found in the webpages into 2 JSON files.
    ...
    for doc_id, doc in tqdm(enumerate(data), total=len(data), desc="Progress"):
        try:
                    ...
                    if not qas["is_impossible"]:
                        for answer_obj in qas["answers"]:
                            try:
                                ...
                                # log failed - qas validation problem
                                with open(
            ...
            # log failed - problem in scraping
...
def create_squad_like_json(output_dir: str):
    ...
    for doc_id, doc_id_group in doc_id_grouped:
        ...
        data.append({"paragraphs": paragraphs, "title": title, "doc_id": int(doc_id[0])}) # Index 0 because doc_id is pd.Series
...
def create_modelling_data(
    ...
    df_qas = df_qas.drop_duplicates(['answer_text','answer_start','answer_end', "qas_id"], keep='last')
